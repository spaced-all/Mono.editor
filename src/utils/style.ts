...
export function deleteStyle(styleContainer: HTMLElement, root: HTMLElement) {
  ...
  if (bchildren.length === 0) {
    const caretPos = nextValidPosition(root);
    setPosition(caretPos, true, true, range);
  } else {
    const left = firstValidPosition(bchildren[0]);
    const right = lastValidPosition(bchildren[bchildren.length - 1]);
    setPosition(left, true, false, range);
    setPosition(right, false, true, range);
  }
...
export function applyStyle(style, root: HTMLElement, range?: Range) {
  ...
  if (contents.childNodes.length === 0) {
    ...
    range.deleteContents();
    // debugger;
    // [text <code>text</code>]<b></b>
    // after deleteContents:
    // [<code></code>]<b></b>
    // so we just delete if
    if (
      !isTag(range.startContainer, "#text") &&
      range.startContainer.childNodes[range.startOffset].textContent === ""
    ) {
    ...
    const left = firstValidPosition(el);
    const right = lastValidPosition(el);
    setPosition(left, true, false, newRange);
    setPosition(right, false, true, newRange);
    sel.removeAllRanges();
    sel.addRange(newRange);
